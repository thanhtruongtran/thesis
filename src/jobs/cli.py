...
class CLIJob:
    ...
    def __init__(
        self, interval=None, period=None, limit=None, end_timestamp=None, retry=True
    ):
        """
        Args:
    ...
    def _get_next_synced_timestamp(self):
        ...
        return (
            round_timestamp(int(time.time()), round_time=self.interval) + self.interval
        )
    ...
    def _check_finish(self, next_synced_timestamp):
        ...
        if (self.end_timestamp is not None) and (
            next_synced_timestamp > self.end_timestamp
        ):
            return True
