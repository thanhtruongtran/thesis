...
async def get_used_requests_and_signals(request: Request):
    ...
    daily_queries = count_db_queries(
        start_time=yesterday_timestamp, end_time=current_time
    )
    number_of_requests = daily_queries.get(today_timestamp, 0) + daily_queries.get(
        yesterday_timestamp, 0
    ) / TimeConstants.A_DAY * (current_time - today_timestamp)
...
async def get_latest_signals(request: Request):
    ...
    return json({"data": signals, "message": "Success"})


@validate(query=WebsocketSignalsQuery)
async def retrieve_signals_via_websocket(
    request: Request, ws: Websocket, query: WebsocketSignalsQuery
):
    r: redis.Redis = request.app.ctx.async_redis
    ws_id = ws.__hash__()
        ...
        try:
            ...
            await ws.send(
                js.dumps(
                    {
                        "error": 400,
                        "message": f"Invalid signal filter {query.filter}. Example to filter signal with value more than $20K and USDC assets: {example_filter}",
                    }
                )
            )
            return
...
async def broadcast_signals(signal, msg=None):
    ...
    for ws_id, ws_info in connected_ws_clients.copy().items():
        try:
            ...
            query = ws_info["query"]
            if not check_signal_by_filter(
                signal,
                chain_id=query["chainId"],
                signal_type=query["type"],
                signal_filter_str=query["filter"],
            ):
                continue
...
async def fetch_last_signals(
    ws: Websocket, r: redis.Redis, query, n_signals=10, timeout=3
):
    last_id = "+"
...
def check_signal_by_filter(
    signal, chain_id=None, signal_type=None, signal_filter_str=None
):
...
async def count_signal_last_24h(r: redis.Redis):
    ...
    values = await asyncio.gather(
        *[r.get(k) for k in filtered_keys], return_exceptions=True
    )
